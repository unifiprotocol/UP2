...
contract Migrator  {
 ...
 //main account set mintRate to Unlimited 0 
    ...
    function extractNative(address _Uptoken,uint multiplier,bool forceCheck)payable  external{
        require(owner == msg.sender,"Unauthorized");
        uint totalSupply = IUP(_Uptoken).totalSupply();
        //IUP(_Uptoken).addMinter(address(this));
        IUP(_Uptoken).updateValues("MintRate",totalSupply*multiplier) ;
        IUP(_Uptoken).mint{
            value: msg.value
        }(address(this),msg.value);
        uint nativeBalance = address(_Uptoken).balance;
        uint redeemPrice =  IUP(_Uptoken).getVirtualPrice();  
        uint noOfUpToBurn = nativeBalance*1e18/redeemPrice;
        if(forceCheck){
            require(noOfUpToBurn <= IUP(_Uptoken).balanceOf(address(this)) ," not enough UP token minted  to ");
        }
    ...
    function mintUP(address upToken) payable external{
        require(owner == msg.sender,"Unauthorized");
        IUP(upToken).mint{
            value: msg.value
        }(address(this),msg.value);
    ...
    function manualBurnUp(address upToken,uint amount) external {
